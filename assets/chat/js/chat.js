...
function chatWith(chatuser, url) {
	url_ = url;
...
function chatHeartbeat(){
	...
	jQuery.ajax({
	  url: url_ + "chat.php?action=chatheartbeat",
	  cache: false,
	  dataType: "json",
...
function closeChatBox(chatboxtitle) {
	...
	jQuery.post(url_+"chat.php?action=closechat", { chatbox: chatboxtitle} , function(data){	
	});
...
function checkChatBoxInputKey(event,chatboxtextarea,chatboxtitle) {
	...
	if(event.keyCode == 13 && event.shiftKey == 0)  {
		...
		if (message != '') {
			jQuery.post(url_+"chat.php?action=sendchat", {to: chatboxtitle, message: message} , function(data){
...
function startChatSession(){  
	jQuery.ajax({
	  url: url_+"chat.php?action=startchatsession",
	  cache: false,
	  dataType: "json",
	  success: function(data) {
